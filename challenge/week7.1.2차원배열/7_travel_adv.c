#include <stdio.h>
#define NUMCITY 3// NUMCITY 3으로 정의
#define NUMPEOPLE 2// NUMPEOPLE 2로 정의
...
int getSum(int numArray[],int length) {
	int totalDays = 0;
	for (int i = 0; i <length; i++) {//i가 length보다 작아질때까지 1씩더함 반복
		totalDays += numArray[i];// numArray[]를 totalDays에 더함
	}
	return totalDays;
}
...
double getAverage(int numArray[],int length) {
	...
	double averageDays = (double)totalDays / length;// averageDays에 totalDays/length 저장
	return averageDays;
}
...
void getFamousCity(double dayArray[],int length) {
	...
	int maxDayIndex = 0;
	for (int i = 0; i < length; i++) {
		if (dayArray[i] > maxDay) {// dayArray배열이 maxDay보다 크면
			maxDay = dayArray[i];// maxDay에 dayArray[i]저장
			maxDayIndex = i;//maxDayIndex에 i저장
		}
	}
